...
for kframe = 1:Nframes % First loop over frames
    ...
    for kfold = 1:K % Then loop over the folds
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            SEval(kfold,klam) = (1/Nval)*(norm(t(valind)-X(valind,:)*what).^2);                         % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
            SEest(kfold,klam) = (1/(N-Nval))*(norm(t(estind)-X(estind,:)*what).^2);                         % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
...
[~,I] = min(RMSEval);          %minimum value and index
lambdaopt = lambdavec(I);     % Assign optimal lambda 
