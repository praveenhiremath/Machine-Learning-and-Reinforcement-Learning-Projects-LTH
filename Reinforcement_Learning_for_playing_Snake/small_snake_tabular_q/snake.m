...
test_agent = true;%false;  %   % set to true when you want to test your agent.
...
rewards = struct('default', 0, 'apple', 100, 'death', -200); % Reward signal
gamm    = 0.99;  % Discount factor in Q-learning
alph    = 0.5;  %0.01; % Learning rate in Q-learning (automatically set to zero during testing)
...
for i = 1 : nbr_ep
    ...
    while 1
        ...
        if terminate
            ...
            sample                    = reward;        %nan % replace nan with something appropriate.
            pred                      = Q_vals(state_idx, action);   %nan % replace nan with something appropriate.
        ...
        % Hint: Q(s,a) <-- (1 - alpha) * Q(s,a) + sample
        % can be rewritten as Q(s,a) <-- Q(s,a) + alpha * (sample - Q(s,a))
        sample                    = reward + (max(Q_vals(next_state_idx,:)))*gamm;    %nan % replace nan with something appropriate
        pred                      = Q_vals(state_idx, action);           %nan % replace nan with something appropriate 
        td_err                    = sample - pred; % don't change this!
        Q_vals(state_idx, action) = Q_vals(state_idx, action) + (alph*td_err);   % + ... (fill in blanks) 
