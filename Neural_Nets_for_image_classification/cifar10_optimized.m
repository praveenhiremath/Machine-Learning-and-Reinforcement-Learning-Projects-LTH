function cifar10_optimized()
    ...
    %Architecture modification (from the following line)
    % Applying 32 (5X5) kernels to the first convolution layer instead of 16 (original) (5X5) kernels
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,3,32)/sqrt(5*5*3/2), 'biases', zeros(32,1)),...
        'padding', [2 2]);
    ...
    % After the 1st conv.layer, adding one more conv.layer with 16 (5X5) kernels with same [2,2] padding     
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,32,16)/sqrt(5*5*32/2), 'biases', zeros(16,1)),...
        'padding', [2 2]);
   ...
   % architecture changed until here
    ...
    net.layers{end+1} = struct('type', 'fully_connected',...
        'params', struct('weights', randn(10,1024)/sqrt(1024/2), 'biases', zeros(10,1)));  % Changed weight matrix from randn(10,4096) to (10,1024)
    ...
    save('models/cifar10_baseline40k_batch50_iters_mu95_2conv_with_32_and_16.mat', 'net');
